...
public class CreationFichier2 {
	...
	public  void menuCreationFichier() throws IOException{
		...
		// recuperation du nm du fichier
		System.out.println("Donner le nom du fichier a creer : ");
		String nomFichier = clavierFich.nextLine();


		//PrintWriter sortie =new PrintWriter (new FileWriter (nomFich)); // formatage du contenu du fichier 

		//FileWriter sortie =new FileWriter(nomFichier+".txt");
	...
	public  void choice2(int choix, File fichier) throws IOException{
		...
		switch(choix){
		case 0:
			System.out.println("Merci d'avoir utilis� notre application, Au revoir.");
			System.exit(0);
			break;

		case 1:
			// ici si on veut ecrire a la suite on est oblig� de mettre un Filwriter avec true en 2 ieme parametres
	...
	public void fichierAjoutGnuplot(ArrayList<Long> temps, ArrayList<Long> temps2, ArrayList<Integer> nbMots) throws IOException{
		...
		//nomFichier="gnuExemple";

		//PrintWriter sortie =new PrintWriter (new FileWriter (nomFichier)); // formatage du contenu du fichier 
		...
		while(i< nbMots.size()){
			...
			sortie.write(nbMots+" "+ligneB+" "+ligneH+"\n");
			i++;
